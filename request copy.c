...
void requestServeDynamic(int fd, char *filename, char *cgiargs, workerThread* currentThread)
{
   ...
   sprintf(buf, "Content-Length: %lu\r\n", 0); //No fucking clue what the length is suppost to be
   sprintf(buf, "%sStat-Req-Arrival:: %lu.%06lu\r\n", buf, currentThread->currentRequest.arrivalTime.tv_sec, currentThread->currentRequest.arrivalTime.tv_usec);
...
{
   int fd = currentThread->currentRequest.connfd;
   int is_static;
   struct stat sbuf;
   ...
   if (strcasecmp(method, "GET")) {
      requestError(fd, method, "501", "Not Implemented", "OS-HW3 Server does not implement this method", currentThread);
      return;
   }
   requestReadhdrs(&rio);
   ...
   if (stat(filename, &sbuf) < 0) {
      requestError(fd, filename, "404", "Not found", "OS-HW3 Server could not find this file", currentThread);
      return;
   }
